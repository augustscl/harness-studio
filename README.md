...
| 版本 | 日期 | 变化 |
|------|------|------|
| v0.6.0 | 2026-09-02 | 插件市场升级 1.40（更新/回滚/快照）+ 成本统计与余额 + 峰谷提示 + 修复压缩包签名 |
| v0.5.0 | 2026-09-01 | 更新检查 + 回合级 token 明细 + 默认中文推理 + 推理力度选择 |
| v0.3.3 | 2026-08-21 | 任务看板（📋 搜索/暂停/继续/停止 goal 任务） |
| v0.3.1 | 2026-08-20 | 引擎升级到官方 DeepSeek Harness v0.1.0-rc.8（本地模型网关修复、图片载荷修复、low 推理强度等） |
| v0.3.0 | 2026-08-18 | 插件市场零依赖安装（内置 pnpm 运行时）+ 系统托盘 + 对齐 dsh-plugin-desktop 契约 |
| v0.2.0 | 2026-08-17 | 插件市场 + 内置5插件（上传/皮肤/产物工坊/交互优化） |
| v0.1.1 | 2026-08-14 | 文件上传入口（含预设图片） |
| v0.1.0 | 2026-08-13 | 第一版：设计→骨架→引擎→桌面壳→发布验证 |

> v0.3.0 要点：插件市场安装/卸载由安装包内置的 pnpm 11.22 独立二进制执行，
> 用户机器上**不需要 Node / pnpm / corepack**；关闭窗口变为收进系统托盘，
> 托盘菜单可显示窗口、重启引擎、打开日志、退出。
>
> v0.3.1 要点：跟随官方 rc.8（官方修复了自定义 OpenAI 兼容网关——我们的
> LM Studio / Ollama 本地模型直接受益——以及图片载荷过大导致请求失败等问题），
> 并适配了 rc.8 的新 profile 目录布局；聊天记录格式不变，升级无感。
